 ...
 * @subpackage Adams
 */
if (!defined('THEME_NAME')) define('THEME_NAME', 'Shiye');
if (!defined('THEME_DB_VERSION')) define('THEME_DB_VERSION', 'v1.1');
    ...
    function biji_ajax_comment_callback()
    {
        ...
        <li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
            <div id="comment-<?php comment_ID(); ?>" class="comment-body new-comment">
                ...
                <div class="comment-meta commentmetadata" style="right:8px;"><a
                            href="<?php echo esc_url(get_comment_link($comment->comment_ID)); ?>"><?php printf(__('%1$s at %2$s'), get_comment_date(), get_comment_time()); ?></a><?php edit_comment_link(__('(Edit)'), ' '); ?>
                </div>
...
$mytheme_update_checker = new ThemeUpdateChecker(
    'Shiye',
    'https://www.shiyest.cn/updates/Shiye.json'
);
?>
...
function zh_cn_l10n_icp_num_callback() {
	echo '<input name="zh_cn_l10n_icp_num" type="text" ' .
		...
		'<p class="description">需要在主题footer.php中设置才能使用。</p>';
}
...
function zh_cn_l10n_icp_num( $content ) {
	if ( defined( 'WP_ZH_CN_ICP_NUM' ) && WP_ZH_CN_ICP_NUM &&
		...
		echo '<a href="https://beian.miit.gov.cn/" rel="nofollow" ' .
			'title="工业和信息化部ICP/IP地址/域名信息备案管理系统" target="_blank">' .
			esc_attr( get_option( 'zh_cn_l10n_icp_num' ) ) .
			 "</a>\n";
	}
}

//页面文件通过do_action( 'admina_credits' )调用函数输出ICP信息 
add_action( 'admina_credits', 'zh_cn_l10n_icp_num' );
?>
...
function beian() {
	add_settings_field( 'baian_num',
		'公安机关备案号',
		'beian_callback',
		'general' );
	register_setting( 'general', 'beian_num' );
}

add_action( 'admin_init', 'beian' );

function beian_callback() {
	$beian_num = esc_attr( get_option( 'beian_num' ) );
	echo '<input name="beian_num" type="text" id="beian_num" value="' .
	     $beian_num .
	     '" class="regluar-text ltr" />' .
	     '<p class="description">需要在主题footer.php中设置才能使用，请填写如：苏公网安备 32010000000000号。</p>';
}

function beian_num( $content ) {
	if ( get_option( 'beian_num' ) ) {
		$beian_num = esc_attr( get_option( 'beian_num' ) );

		// 获取备案号中的数字
		$arr      = [];
		$patterns = "/\d+/"; //第一种
		preg_match_all( $patterns, $beian_num, $arr );
		$code = implode( $arr[0] );

		// 警徽图标，定义一个编码为Base64的二进制图片，可以用后面网址转换：http://tool.chinaz.com/tools/imgtobase/
		$images = 'data:image/png;base64,';
...
//页面文件通过do_action( 'adminb_credits' )调用函数输出公安网备信息 
add_action( 'adminb_credits', 'beian_num' );
?>
